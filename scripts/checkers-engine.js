...
function printInvalidMoveError(message) {
    d3.select("#currentLog")
            .style("color", "red")
            .html(message)
}

function isMoveLegal(cells, pieces, piece, from, to) {
	if (currentBoard.turn === player && dragEnd === true && piece.state === -1) {
		printInvalidMoveError("Movimento inválido. As peças pretas não são do Jogador.");
		return false;
	}
    if ((to === undefined) || (to === null)  || (to.col < 0) || (to.row < 0) || (to.col > 7) || (to.row > 7)) {
        if (currentBoard.turn === player && dragEnd === true) printInvalidMoveError("Movimento inválido. Peça indo para fora do tabuleiro.");
        return false;
    }
    var distance = {x: to.col-from.col,y: to.row-from.row};
    if ((distance.x == 0) || (distance.y == 0)) {
        if (currentBoard.turn === player && dragEnd === true) printInvalidMoveError("Movimento inválido. Não é possível realizar movimento vertical ou horizontal.");
        return false;
    }
    if (abs(distance.x) != abs(distance.y)) {
    	if (currentBoard.turn === player && dragEnd === true) printInvalidMoveError("Movimento inválido. Não é possível realizar movimento não diagonal.");
        return false;
    }
    if (abs(distance.x) > 2) {
        if (currentBoard.turn === player && dragEnd === true) printInvalidMoveError("Movimento inválido. Não é possível andar mais de duas casas de uma vez.");
        return false;
    }
    if (to.state != empty) {
        if (currentBoard.turn === player && dragEnd === true) printInvalidMoveError("Movimento inválido. A casa escolhida não está vazia.");
        return false;
    }
    if (abs(distance.x) == 2) {
    	var jumpedPiece = getJumpedPiece(cells, pieces, from, to);
    	if (jumpedPiece == null) {
            if (currentBoard.turn === player && dragEnd === true) printInvalidMoveError("Movimento inválido. Sem peças para pular sobre.");
    		return false;
    	}
		var pieceState = integ(piece.state);
		var jumpedState = integ(jumpedPiece.state);
        if (pieceState != -jumpedState) {
            if (currentBoard.turn === player && dragEnd === true) printInvalidMoveError("Movimento inválido: Não é possível pular sobre a própria peça.");
        	return false;
    	}
    }
    if ((integ(piece.state) === piece.state) && (sign(piece.state) != sign(distance.y))) {
        if (currentBoard.turn === player && dragEnd === true) printInvalidMoveError("Movimento inválido: Esta peça não pode ir para esta direção.");
        return false;
    }
...
function alpha_beta_search(calc_board, limit) {
    ...
    available_moves.forEach((move) => {
        ...
        const pieceIndex = getPieceIndex(simulated_board.pieces, move.from.row, move.from.col);
        const piece = simulated_board.pieces[pieceIndex];
        movePiece(simulated_board, piece, move.from, move.to);


        const score = min_value(simulated_board, get_available_moves(player, simulated_board), limit - 1, alpha, beta);
        move.score = score;
    ...
    return best_moves.length > 1 ? select_random_move(best_moves) : best_moves[0];
}

function computerMove() {
    var simulated_board = copy_board(currentBoard);

    var selected_move = alpha_beta_search(simulated_board, 4);
...
function min_value(calc_board, human_moves, limit, alpha, beta) {
    ...
    human_moves.forEach((move) => {
        ...
        const score = max_value(simulated_board, get_available_moves(computer, simulated_board), limit - 1, alpha, beta);
        move.score = score;
